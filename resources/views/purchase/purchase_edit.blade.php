...
<div class="modal-body">
                ...
                <div class="row">
                    <div class="col-lg-4">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-5">
                                <input type="text" name="purchase_date" class="form-control today" id="purchase_date" value="{{ $purchase->purchase_date }}" style="width: 108px;">
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-5">
                                <input type="text" name="purchase_duedate" class="form-control today" id="purchase_duedate" value="{{ $purchase->purchase_duedate }}" style="width: 108px;">
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                    ...
                    <div class="col-lg-4">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <textarea class="form-control" name="note" id="note" rows="3" style="resize: none;">{{ $purchase->note }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table table-bordered text-nowrap">
                                ...
                                <tbody id="product_list">
                                    ...
                                    <tr>
                                        <td style="text-align: center; width: 0px;">
                                            {{ $loop->iteration }}
                                            <input type="hidden" name="no_urut[]" id="no_urut_{{ $loop->iteration }}" class="no_urut" value="{{ $loop->iteration }}" readonly>
                                        </td>
                                        <td style="text-align: center; width: 72px;">
                                            @if ($loop->iteration > 1)
                                            <button class="btn btn-danger btn-sm" type="button" onclick="delete_row(this,{{ $loop->iteration }})" title="Hapus Baris"><i class="fas fa-trash"></i></button>
                                            @endif
                                        </td>
                                        <td>
                                            <select class="form-control" name="product_code_alias[]" id="product_code_alias_{{ $loop->iteration }}" onchange="double_product_check('{{ $loop->iteration }}')" style="width: 400px;">
                                                <option value="">-PILIH-</option>
                                                @foreach($products as $product)
                                                    <option value="{{ $product->product_code_alias }}" {{ ($item->product_code_alias == $product->product_code_alias) ? 'selected' : '' }}>{{ $product->product_variant_name }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td>
                                            <span style="" id="unit_name_{{ $loop->iteration }}">{{ $item->name }}</span>
                                            <input type="hidden" name="unit_id[]" id="unit_id_{{ $loop->iteration }}" value="{{ $item->unit_id }}">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control" name="purchase_qty[]" id="purchase_qty_{{ $loop->iteration }}" value="{{ number_format($item->purchase_qty) }}" onkeyup="qty_add(this, {{ $loop->iteration }})" style="text-align: right; width: 101px;">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control" name="purchase_price[]" id="purchase_price_{{ $loop->iteration }}" value="{{ number_format($item->purchase_price) }}" onkeyup="price_add(this, {{ $loop->iteration }})" style="text-align: right; width: 110px;">
                                        </td>
                                        <td style="text-align: right;">
                                            <span id="set_amount_{{ $loop->iteration }}">{{ number_format($item->amount) }}</span>
                                            <input type="hidden" name="amount[]" id="amount_{{ $loop->iteration }}" value="{{ $item->amount }}">
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot style="text-align: right;">
                                    ...
                                    <tr>
                                        ...
                                        <th colspan="1">
                                            <div class="custom-control custom-checkbox col-sm-12">
                                                <input name="ppn_check" id="ppn_check" class="custom-control-input" type="checkbox" onclick="set_ppn()" {{ ($purchase->ppn > 0) ? "checked" : "" }}>
                                    ...
                                    <tr>
                                        ...
                                        <th>
                                            <input type="text" class="form-control" name="discount" id="discount" value="{{ number_format($purchase->discount) }}" style="text-align: right; width: 101px; display: inline-block;" onkeyup="set_total_all(this)">
                                        </th>
                                    </tr>
                                    <tr>
                                        <th colspan="5"></th>
                                        ...
                                        <th>
                                            <input type="text" class="form-control" name="delivery" id="delivery" value="{{ number_format($purchase->delivery) }}" style="text-align: right; width: 101px; display: inline-block;" onkeyup="set_total_all(this)">
                                        </th>
                                    </tr>
                                    <tr>
                                        <th colspan="5"></th>
    ...
    <div class="modal-footer">
        <button type="button" class="btn btn-primary" onclick="purchase_update('edit')">Simpan Perubahan</button>
        <button type="button" class="btn btn-outline-primary" onclick="purchase_update('po')">Proses PO</button>
...
<script>   
    ...
    function double_product_check(urut) {
        ...
        $.post("{{ url('/purchase/double_product_check') }}", $("#form_edit").serialize()+'&get_product_code='+product_code_alias, function(result) {
    ...
    function purchase_update(process_type) {        
        ...
        $.post("{{ url('/purchase/purchase_update') }}", $("#form_edit").serialize()+'&process_type='+process_type, function(result) {
            setTimeout(function () {
                $('.overlay').prop('hidden', true);
            }, 1000);
            
            if(result.status=="ok") {
                success_notif('Data pembelian berhasil diupdate');
                close_modal_xl_load();
            } else {
                error_notif('Data pembelian gagal diupdate');
                $('.overlay').prop('hidden', false);
            }
